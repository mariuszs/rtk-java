...
///    catches the forms this gate lets past (see #3980). In this fork a
///    heredoc is the exception: see the keep comment in the body.
...
pub(crate) fn decide_with_params(
    ...
    // Fork keep (standing exception in CLAUDE.md): upstream refuses every
    // heredoc here and in `rewrite_command`. The fork rewrites what follows
    // the terminator instead — the body is inert data — because the agent
    // idiom `python3 - <<'PY' … PY` + `./mvnw test` otherwise hands raw Maven
    // to the agent (91 calls, 76k chars, measured 2026-09-11). The permission
    // splitter cannot tell a body line from a command, so such a rewrite is
    // never auto-allowed: the agent is asked exactly as for the raw command.
...
mod tests {
    ...
    #[test]
    fn heredoc_followed_by_a_build_is_rewritten_but_only_asked() {
        // Fork keep. 2026-09-11 traffic: `python3 - <<'PY' … PY` +
        // `./mvnw test | tail` deferred whole, so raw Maven reached the agent.
        // It rewrites — and stays an Ask even on an Allow verdict, because the
        // permission splitter reads heredoc body lines as if they were commands.
    ...
    #[test]
    fn heredoc_with_a_file_redirect_still_defers() {
        assert_eq!(
            decide_with_params(
                "cat <<'EOF' > /tmp/x\nfoo\nEOF",
