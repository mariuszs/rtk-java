...
pub const RULES: &[RtkRule] = &[
    ...
    // Only DIRECT tool invocations map to `rtk lint` (rtk re-runs the tool
    // with `-f json`, args preserved). Script forms (`npm run lint`,
    // `npm lint`, bare `lint`) run an arbitrary user-defined script whose
    // body is unknown — mapping them here replaced the real script with
    // `eslint .` and produced false lint failures on real projects
    // (2026-07-10). They now fall through to the generic npm/pnpm/npx rules.
    RtkRule {
        pattern: r"^((p?npx|npm\s+(exec|x)|pnpm\s+(exec|dlx))\s+)?(biome|eslint)(\s|$)",
        rtk_cmd: "rtk lint",
        pipeline_safety: PipelineSafety::ProducerOnly,
        rewrite_prefixes: &[
            "biome",
    ...
    RtkRule {
        ...
        //
        // One rule, not two: `rtk_cmd` is the key `rewrite_single` looks the
        // rule up by, so a second `rtk mvnd` entry would shadow this one and
        // strand its `rewrite_prefixes`.
        pattern: r"^(?:mvnd\.cmd|mvnd)\b(?:\s+\S+)*?\s+(test|verify|compile|package|clean|install|integration-test|deploy|dependency:tree|checkstyle:check|checkstyle|failsafe:integration-test|failsafe:verify|surefire:test|dependency:list|process-classes)\b",
        rtk_cmd: "rtk mvnd",
