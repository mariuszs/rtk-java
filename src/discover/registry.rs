...
static TRUNCATION_STAGE: LazyLock<Regex> = LazyLock::new(|| {
...
pub(crate) fn rewrite_command_precompiled(
    ...
    if let Some(rewritten) = rewrite_shell_dash_c(trimmed, compiled, normalized_prefixes) {
...
fn rewrite_shell_dash_c(
    ...
    let rewritten = rewrite_command_precompiled(inner, excluded, transparent_prefixes)?;
    ...
    for tok in &tokens {
        ...
        match tok.kind {
            ...
            TokenKind::Pipe(_) => {
                ...
                // `mvn … | tail -3`: the truncation stages exist to tame raw
                // Maven output, so after the mvn filter they would only cut the
                // compact summary. Rewrite the head stage and drop the group.
                // Deliberately narrow — `rtk mvn` plus numeric truncation stages
                // only — so no other pipeline loses data to the rewrite.
                let head_stage = cmd[seg_start..tok.offset].trim();
                let mvn_truncation_only =
                    if is_truncation_only_pipe_group(&cmd[tok.offset..analysis.end_offset]) {
                        rewrite_segment(head_stage, excluded, transparent_prefixes)
                            .filter(|rewritten| is_rtk_mvn_invocation(rewritten))
                    } else {
                        None
                    };

                let rewritten_pipeline = mvn_truncation_only.or_else(|| {
...
static TIMEOUT_DURATION_RE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^\d+(\.\d+)?[smhd]?$").unwrap());
...
fn rewrite_segment_inner(
        ...
        && (cmd_part.starts_with("head -") || cmd_part.starts_with("tail "))
    {
        ...
        // file reads get summarized by `rtk read`. Any sudo/env prefix has
        // already been peeled by strip_disabled_prefix above.
    ...
    let rtk_equivalent = match classify_command(cmd_part) {
        ...
        Classification::Unsupported { .. } => {
            ...
            // A path-qualified command reached through a peeled wrapper stays
            // raw. `/usr/bin/jj log` on its own rewrites fine, but once a
            // prefix was stripped to get here the rewrite would re-emit the
            // wrapper around a form the wrapper never saw.
            if depth > 0
                && cmd_part
                    .split_whitespace()
                    .next()
...
mod tests {
    ...
    fn test_mvn_pipe_with_grep_not_rewritten() {
        ...
        // since the downstream stage would then parse filtered text.
        assert_eq!(
    ...
    #[test]
    fn test_mvn_tail_follow_not_rewritten() {
        // `tail -f` is a follow, not a truncation stage: the group stays, which
        // leaves mvn as a non-final stage and therefore unrewritten.
        assert_eq!(
    ...
    #[test]
    fn test_non_mvn_trailing_tail_not_rewritten() {
        // The truncation-drop rule is mvn-only; every other head stage keeps its
        // raw output, because the pipe consumer expects the real command's text.
        assert_eq!(
    ...
    #[test]
    #[ignore = "fork does not target the Windows mvnw.cmd wrapper"]
    ...
    #[test]
    fn test_classify_mvn_clean_supported() {
        // Fork flagship: `clean` IS filtered (95% savings) where upstream bypasses it.
        assert!(matches!(
    ...
    #[test]
    fn test_classify_mvn_dependency_tree_supported() {
        // Fork flagship: `dependency:tree` IS filtered (70% savings) where upstream bypasses it.
        assert!(matches!(
