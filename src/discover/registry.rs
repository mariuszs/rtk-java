    ...
    for tok in &tokens {
        ...
        match tok.kind {
            ...
            TokenKind::Pipe(_) => {
                ...
                // only — so no other pipeline loses data to the rewrite. Every
                // other producer goes through upstream's `PipelineProducer`
                // path below, which keeps the consumer stages intact.
                ...
                let rewritten_pipeline = mvn_truncation_only
                    .or_else(|| {
...
mod tests {
    ...
    #[test]
    fn test_non_mvn_trailing_tail_keeps_its_consumer() {
        // The truncation-*drop* rule is mvn-only. Every other producer goes
        // through upstream's `PipelineProducer` path (#3171), which rewrites
        // the head stage but leaves the consumer stages verbatim.
        assert_eq!(
