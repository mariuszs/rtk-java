...
impl BlockHandler for TscHandler {
    ...
    ///
    /// Two cases still need a line rtk composes itself: pretty mode's
    /// `Found 3 errors in the same file, starting at: src/index.ts:1`, which
    /// names a location instead of a file count and so cannot stand in for the
    /// verdict; and a compiler that failed without printing one diagnostic rtk
    /// understands, where silence would read as a clean run.
    fn format_summary(&self, exit_code: i32, raw: &str) -> Option<String> {
        ...
        if self.error_count == 0 {
            ...
            // no project, unparseable output). Silence would be a false green,
            // so surface the failure with both ends of the raw output.
...
";
        ...
        assert!(
            result.contains("Found 3 errors in 2 files"),
            "got: {}",
            result
        );
        assert!(
