...
- **Single-goal routing** (`dispatch` → `route_goal`): `test`, `verify`, and the lifecycle goals that run the test phase (`integration-test`, `package`, `install`, `deploy`) route to the shared surefire/failsafe state-machine filter (+ XML enrichment), each running its OWN goal name (not `verify`); `clean`, compile-likes, `checkstyle`/`checkstyle:check`, and `dependency:tree` route to their dedicated filters; any other goal (e.g. `spring-boot:run`, `quarkus:dev`) streams via unfiltered `cmd.status()` passthrough — safe for long-running goals. For every filtered single-goal route, `-q`/`--quiet` is stripped (via `filtered_goal_args`) so rtk receives full output and does the compression itself — the same "smart quiet" as multi-goal; the Passthrough route keeps `-q` since it streams raw. _Future: a thin lifecycle-tail keep-rule (`Building jar`, `Installing … to ~/.m2`, `Uploading …` + artifact path) could surface package/install/deploy signal the test filter currently drops — preferred over porting upstream's `filter_package`, which lacks XML enrichment._
