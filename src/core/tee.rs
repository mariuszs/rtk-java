...
/// Force-write `content` as an auxiliary artifact and return a bare reference
/// to it — a display path under legacy tee, `rtk recall <hash>` under the
/// sqlite store — or None when recovery is disabled or the write fails. Used
/// for auxiliary artifacts like the mvn class digest, which embed the
/// reference in their own sentence instead of a `[full output: ...]` hint.
pub fn force_tee_display(content: &str, command_slug: &str) -> Option<String> {
    ...
    match mode {
        RecoveryMode::Disabled => None,
        RecoveryMode::Tee => super::tee_file::force_tee_display(&cfg, content, command_slug)
            .inspect(|_| retriever::record_tee_elision(&cfg, command_slug)),
        RecoveryMode::Sqlite => {
            match retriever::store(&cfg, content.as_bytes(), command_slug, None, 1) {
                Stored::Saved(s) => Some(format!("rtk recall {}", s.hash)),
                Stored::Unavailable | Stored::Empty => None,
            }
        }
    }
