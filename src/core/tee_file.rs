...
/// Bare display path for an auxiliary artifact (no `[full output: ...]`
/// wrapper) — the caller embeds it in its own sentence.
pub fn force_tee_display(cfg: &RetrieverConfig, content: &str, slug: &str) -> Option<String> {
    let path = write(cfg, content, slug)?;
    Some(display_shell_path(&path))
}
