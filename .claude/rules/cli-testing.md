...
The `mvn dependency:*` filters are the documented exception to that floor — their thresholds
were measured against real fixtures and deliberately kept low to preserve fidelity:

| Filter | Measured floor | Rationale |
|--------|----------------|-----------|
| `mvn dependency:list` | 10%+ | Fidelity exception (2026-07-09): verbatim native subset keeps every resolved line — savings come only from boilerplate removal |
